...
public class Game implements Runnable {
    ...
    private int score;              //defines player score. 
    private ArrayList<Enemy> enemies; // To store an enemies colection
    private ArrayList<Brick> bricks; // my bricks
         ...
         for(int i = 0; i < 7; i++){
             for(int j = 0; j < 5; j++){
                 ...
                 Enemy enemy = new Enemy(i * width_enemy + 2,30 * j +5, 
                         width_enemy - 20, 25, VelocityEnemies, this); 
                 enemies.add(enemy);
             }
         }
         
         //inits blockade for player
         bricks = new ArrayList<Brick>();
         for (int k = 0; k <= 4; k++) {
            for (int i = 0; i < 4; i++) {
                for (int j = 0; j < 4; j++) {
                    int width_brick = getWidth() / 10 ;
                    Brick brick = new Brick(i * 25 + (2 * k * 100) + (50) , ((6 * (getHeight()/10)) - 5) + j * 10, 25, 10, this);
                    bricks.add(brick);
             }
         }
         }
                        ...
                        for(int i = 0; i < 7; i++){
                            for(int j = 0; j < 5; j++){
                                ...
                                enemies.add(new Enemy(i * width_enemy + 2, 30 * j +5, 
                                    width_enemy - 20, 25, enemy.getVelocity(),this)); 
                                enemies.add(enemy);
                            }
                        }
                    }
    ...
    private void render() {
        ...
        */
        ...
        else
        {
            ...
            player.render(g);
            g.setColor(lightGray);// to color the string that says pause
            g.setFont(new Font("Avenir Next", Font.PLAIN, 20));//set font
            g.drawString("Score: " + score, getWidth() - 100, 20);// display the string in the middle of screen
